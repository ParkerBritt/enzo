...
PrimitiveTreeModel::PrimitiveTreeModel(QObject* parent) : QAbstractItemModel(parent)
{
...
{
    if (orientation == Qt::Horizontal && role == Qt::DisplayRole) return rootItem_->data(section);
...
QModelIndex PrimitiveTreeModel::parent(const QModelIndex& index) const
{
    ...
    if (!parentItem || parentItem == rootItem_.get()) return {};
...
PrimitiveTreeItem*
PrimitiveTreeModel::findOrCreateChild(PrimitiveTreeItem* parent, const std::string& name)
{
    QString qname = QString::fromStdString(name);
    for (int i = 0; i < parent->childCount(); ++i)
    {
        if (parent->child(i)->data(0).toString() == qname) return parent->child(i);
    }
