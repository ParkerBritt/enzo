...
{
    if (number < 0 || number >= childCount()) return nullptr;
    return childItems_[number].get();
}

int PrimitiveTreeItem::childCount() const { return static_cast<int>(childItems_.size()); }

int PrimitiveTreeItem::columnCount() const { return static_cast<int>(itemData_.size()); }
...
{
    if (position < 0 || position > columnCount()) return false;
...
PrimitiveTreeItem* PrimitiveTreeItem::parent() { return parentItems_; }

bool PrimitiveTreeItem::removeChildren(int position, int count)
{
    if (position < 0 || position + count > childCount()) return false;
...
{
    if (position < 0 || position + columns > columnCount()) return false;
