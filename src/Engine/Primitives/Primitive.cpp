...
geo::Primitive::getAttributes(attr::AttributeOwner owner, bool includeIntrinsics) const
{
...
std::vector<std::shared_ptr<const attr::Attribute>>
geo::Primitive::getGroups(attr::AttributeOwner owner) const
{
    const auto& store = getGroupStore(owner);
...
// Copies one element's value from the source attribute into the destination attribute.
template <typename T>
void copyAttributeValue(const AttributePair& pair, Offset sourceOffset, Offset destOffset)
{
...
// Writes a blend of two elements' values from the source attribute into the destination attribute.
template <typename T>
void interpolateAttributeValue(
...
{
    attr::AttributeHandleRO<T> sourceHandle(pair.source);
    const T blended = sourceHandle.getValue(sourceOffset0) * (1.0 - blend) +
                      sourceHandle.getValue(sourceOffset1) * blend;
    attr::AttributeHandle<T>(pair.dest).setValue(destOffset, blended);
}

} // namespace

void geo::Primitive::addAttributesFrom(const Primitive& source, attr::AttributeOwner owner)
{
    for (const auto& sourceAttribute : source.getAttributes(owner))
    {
        if (attributeExists(owner, sourceAttribute->getName())) continue;
        addAttribute(owner, sourceAttribute->getName(), sourceAttribute->getType());
    }
    for (const auto& sourceGroup : source.getGroups(owner))
        addGroup(owner, sourceGroup->getName());
}

void geo::Primitive::copyAttributeValuesFrom(
...
{
    for (const AttributePair& pair : getAttributePairs(source, *this, owner))
    {
        switch (pair.source->getType())
        {
        case attr::AttributeType::intT:
            copyAttributeValue<intT>(pair, sourceOffset, destOffset);
            break;
        case attr::AttributeType::floatT:
            copyAttributeValue<floatT>(pair, sourceOffset, destOffset);
            break;
        case attr::AttributeType::vectorT:
            copyAttributeValue<Vector3>(pair, sourceOffset, destOffset);
            break;
        case attr::AttributeType::boolT:
            copyAttributeValue<boolT>(pair, sourceOffset, destOffset);
            break;
        case attr::AttributeType::matrixT:
            copyAttributeValue<Matrix4>(pair, sourceOffset, destOffset);
            break;
        default:
            break;
        }
...
void geo::Primitive::interpolateAttributeValuesFrom(
    const Primitive& source,
    attr::AttributeOwner owner,
    Offset sourceOffset0,
    Offset sourceOffset1,
...
{
    const Offset nearerSourceOffset = blend < 0.5 ? sourceOffset0 : sourceOffset1;
    ...
    for (const AttributePair& pair : getAttributePairs(source, *this, owner))
    {
        ...
        case attr::AttributeType::intT:
        {
            ...
            const double blended = sourceHandle.getValue(sourceOffset0) * (1.0 - blend) +
                                   sourceHandle.getValue(sourceOffset1) * blend;
            attr::AttributeHandle<intT>(pair.dest).setValue(
                destOffset,
                static_cast<intT>(std::llround(blended))
            );
            break;
        }
        case attr::AttributeType::floatT:
            interpolateAttributeValue<floatT>(
                pair,
                sourceOffset0,
                sourceOffset1,
                blend,
            ...
            break;
        case attr::AttributeType::vectorT:
            interpolateAttributeValue<Vector3>(
                pair,
                sourceOffset0,
                sourceOffset1,
                blend,
            ...
            break;
        case attr::AttributeType::boolT:
            copyAttributeValue<boolT>(pair, nearerSourceOffset, destOffset);
            break;
        case attr::AttributeType::matrixT:
            copyAttributeValue<Matrix4>(pair, nearerSourceOffset, destOffset);
            break;
        default:
            break;
        }
