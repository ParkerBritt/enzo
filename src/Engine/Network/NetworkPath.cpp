...
NetworkPath::NetworkPath(const char* path)
    : NetworkPath(std::string(path))
{
}
...
bool NetworkPath::isValidFormatting(const std::string& pathString)
{
    ...
    if (dotPos == std::string::npos)
        return Path::isValidFormatting(pathString);
...
bool NetworkPath::isValid() const
{
    return isValidFormatting(path_);
}

bool NetworkPath::hasParameter() const
{
    return parameterDelimiter(path_) != std::string::npos;
}
