...
bool PathSelectionComponent::containsFace(
...
) const
{
    ...
    if (isWholePrim(prim)) return !inverted;
    if (!faces_) return false;
    bool in = faces_->contains(index);
    return inverted ? !in : in;
}

bool PathSelectionComponent::containsPoint(
    const geo::Primitive& prim,
    Index index,
    Offset /*offset*/,
    bool inverted
...
bool PathSelectionComponent::containsVertex(
    const geo::Primitive& prim,
    Index index,
    Offset /*offset*/,
    bool inverted
...
bool GroupSelectionComponent::containsPrim(const geo::Primitive& prim) const
{
    ...
    {
        if (prim.getGroupByName(owner, groupName_)) return true;
    }
    return false;
}

bool GroupSelectionComponent::containsFace(
    const geo::Primitive& prim,
    Index /*index*/,
    Offset offset,
    bool inverted
) const
{
    ...
    if (!prim.getGroupByName(attr::AttrOwner::FACE, groupName_)) return false;
    bool in = isGroupMember(prim, attr::AttrOwner::FACE, groupName_, offset);
    return inverted ? !in : in;
}

bool GroupSelectionComponent::containsPoint(
    const geo::Primitive& prim,
    Index /*index*/,
    Offset offset,
    bool inverted
) const
{
    if (!containsPrim(prim)) return false;
    if (isWholePrim(prim)) return !inverted;
    if (!prim.getGroupByName(attr::AttrOwner::POINT, groupName_)) return false;
    bool in = isGroupMember(prim, attr::AttrOwner::POINT, groupName_, offset);
    return inverted ? !in : in;
}

bool GroupSelectionComponent::containsVertex(
    const geo::Primitive& prim,
    Index /*index*/,
    Offset offset,
    bool inverted
) const
{
    if (!containsPrim(prim)) return false;
    if (isWholePrim(prim)) return !inverted;
    if (!prim.getGroupByName(attr::AttrOwner::VERTEX, groupName_)) return false;
