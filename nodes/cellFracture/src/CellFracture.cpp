...
)
{
    ...
    for (size_t otherSeedIndex = 0; otherSeedIndex < seedPositions.size(); ++otherSeedIndex)
    {
        ...
        if (otherSeedToSeed.squaredNorm() == 0)
        {
            if (otherSeedIndex < seedIndex) return std::make_shared<geo::Mesh>(mesh.getPath());
            continue;
        }
        const Vector3 towardSeed = otherSeedToSeed.normalized();

        const Vector3 midpoint = (seedPosition + otherSeedPosition) / 2;
        const utils::PlaneCut cut = utils::cutMeshByPlane(*uncut, midpoint, towardSeed);
...
)
{
    ...
    for (size_t usedIndex = 0; usedIndex < usedPiecePoints.size(); ++usedIndex)
    {
        fractured.copyAttributeValuesFrom(
            piece,
            attr::AttributeOwner::POINT,
            usedPiecePoints[usedIndex],
            fracturedPoints[usedIndex]
        );
    }
    ...
    for (size_t faceIndex = 0; faceIndex < pieceFaces.size(); ++faceIndex)
    {
        ...
        const Offset fracturedFace = fracturedFaces[faceIndex];
        fractured
            .copyAttributeValuesFrom(piece, attr::AttributeOwner::FACE, pieceFace, fracturedFace);

        const Offset pieceStartVertex = piece.getFaceStartVertices()[pieceFace];
        const Offset fracturedStartVertex = fractured.getFaceStartVertices()[fracturedFace];
        for (Offset cornerIndex = 0; cornerIndex < piece.getFaceVertCount(pieceFace); ++cornerIndex)
        {
            fractured.copyAttributeValuesFrom(
                piece,
                attr::AttributeOwner::VERTEX,
                pieceStartVertex + cornerIndex,
                fracturedStartVertex + cornerIndex
            );
        }
    }
